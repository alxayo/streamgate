  ...
  router.delete('/admin/cache/:eventId', asyncHandler(async (req: Request, res: Response) => {
    ...
    if (upstreamProxy) {
      try {
        ...
        console.error(`[admin-cache] Failed to delete upstream blobs for ${eventId}:`, error);
      }
    }

    console.log(`[admin-cache] Purge complete for ${eventId}: deletedBlobs=${deletedBlobs}`);
    res.json({ deletedCache: true, deletedBlobs });
  }));
